...
from openpoints.utils import set_random_seed, save_checkpoint, load_checkpoint, resume_checkpoint1, setup_logger_dist, \
...
def main(gpu, cfg):
    ...
    if cfg.pretrained_path is not None:
        if cfg.mode == 'resume':
            best_ins_miou = resume_checkpoint1(cfg, model, optimizer, scheduler,
            ...
            if cfg.mode in ['val', 'test']:
                ...
                with np.printoptions(precision=2, suppress=True):
                        logging.info(f'Voting Instance mIoU {test_ins_miou:.2f}, '
    ...
    if cfg.get('num_votes', 0) > 0:
        best_epoch, _ = load_checkpoint(model, pretrained_path=os.path.join(
            cfg.run_dir, f'{cfg.run_name}_ckpt_best.pth'))
...
def train_one_epoch(model, train_loader, criterion, optimizer, scheduler, epoch, cfg):
    ...
    pbar = tqdm(enumerate(train_loader), total=train_loader.__len__(), ncols=0)
    num_iter = 0
...
def validate(model, val_loader, cfg, num_votes=0, data_transform=None):
    ...
    pbar = tqdm(enumerate(val_loader), total=val_loader.__len__(), ncols=0)
