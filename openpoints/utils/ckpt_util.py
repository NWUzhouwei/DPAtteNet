...
def resume_model(model, cfg, pretrained_path=None):
    pretrained_path = os.path.join(cfg.run_dir, os.path.join(cfg.run_name,
...
def save_checkpoint(cfg, model, epoch, optimizer=None, scheduler=None,
    ...
    current_pretrained_path = os.path.join(cfg.run_dir, current_ckpt_name) # cfg.ckpt_dir
    save_dict = {
    ...
    if cfg.save_freq > 0 and epoch % cfg.save_freq == 0:
        ...
        milestone_pretrained_path = os.path.join(cfg.run_dir, milestone_ckpt_name)
    ...
    if is_best:
        ...
        best_pretrained_path = os.path.join(cfg.run_dir, best_ckpt_name)
...
def resume_checkpoint1(config, model, optimizer=None, scheduler=None, pretrained_path=None, printer=logging.info):
    if pretrained_path is None:
        pretrained_path = config.pretrained_path
        assert pretrained_path is not None
    printer("=> loading checkpoint '{}'".format(pretrained_path))
    ...
    config.epoch = checkpoint['epoch'] + 1
    printer("=> loaded successfully '{}' (epoch {})".format(pretrained_path, checkpoint['epoch']))
    best_ins_miou = checkpoint['ins_miou']
    del checkpoint
    torch.cuda.empty_cache()
    return best_ins_miou
