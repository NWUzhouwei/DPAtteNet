...
from .dpattention import Attention, TransformerBlock, InvResAtten
...
class SetAbstraction(nn.Module):
    ...
    def __init__(self,
        ...
        if is_head:
            self.first_tf = TransformerBlock(cin=in_channels, cout=out_channels // 2, group_args=group_args)
            self.second_tf = TransformerBlock(cin=out_channels // 2, cout=out_channels, group_args=group_args)
        else:
            self.tf = TransformerBlock(cin=in_channels, cout=in_channels, group_args=group_args)
            if self.all_aggr:
                self.all_tfextra = TransformerBlock(cin=in_channels, cout=out_channels, group_args=group_args)
                self.pool = lambda x: torch.max(x, dim=-1, keepdim=False)[0]
            else:
                self.neighborTF = TransformerBlock(in_channels, out_channels, group_args, stride, sampler)

    def forward(self, pf):
        # p:[b,n,3] f:[b,c,n]
        p, f = pf
        if self.is_head:
            p, f = self.first_tf(p, f.contiguous()) # [b,c,n]
            p, f = self.second_tf(p, f.contiguous()) # [b,c,n]
        else:
            p, f = self.tf(p, f.contiguous()) # [b,c,n]
            if self.all_aggr:
                p, f = self.all_tfextra(p, f.contiguous())
                f = self.pool(f)
                return p, f
                
            p, f = self.neighborTF(p, f) # [b,m,3] [b,c,m]
...
class FeaturePropogation(nn.Module):
    ...
    def __init__(self, mlp,
        ...
        """
        super().__init__()
        self.tf = TransformerBlock(cin=mlp[0], cout=mlp[2], group_args=group_args)

    def forward(self, pf1, pf2=None):
        p1, f1 = pf1
        p2, f2 = pf2
        if f1 is not None:
            f = torch.cat((f1, three_interpolation(p1, p2, f2)), dim=1) # [b,c,n]
            _, f = self.tf(p1, f.contiguous()) # [b,c,n]
        else:
...
class DPAtteNetEncoder(nn.Module):
    def __init__(self,
                 in_channels: int = 4,
                 width: int = 32,
                 blocks: List[int] = [1, 4, 7, 4, 4],
                 strides: List[int] = [4, 4, 4, 4],
                 block: str or Type[InvResAtten] = 'InvResAtten',
...
@MODELS.register_module()
class DPAtteNetPartDecoder(nn.Module):
    def __init__(self,
                 encoder_channel_list: List[int], # self.encoder.channel_list
                 decoder_layers: int = 2,
                 decoder_blocks: List[int] = [1, 1, 1, 1],
                 decoder_strides: List[int] = [4, 4, 4, 4],
                 act_args: str = 'relu',
                 cls_map='pointnet2', # curvenet
                 num_classes: int = 16,
                 cls2partembed=None,
                 **kwargs
                 ):
        ...
        self.in_channels = encoder_channel_list[-1] # 2560
        skip_channels = encoder_channel_list[:-1] # [160, 320, 640, 1280]
        fp_channels = encoder_channel_list[:-1] # [160, 320, 640, 1280]
        
        # the following is for decoder blocks
        self.conv_args = kwargs.get('conv_args', None)
        radius_scaling = kwargs.get('radius_scaling', 2)
        nsample_scaling = kwargs.get('nsample_scaling', 1)
        block = kwargs.get('block', 'InvResAtten')
        if isinstance(block, str):
            block = eval(block)
        ...
        self.norm_args = kwargs.get('norm_args', {'norm': 'bn'}) 
        self.act_args = kwargs.get('act_args', {'act': 'relu'}) 
        self.expansion = kwargs.get('expansion', 4) # 4
        radius = kwargs.get('radius', 0.1)
        nsample = kwargs.get('nsample', 16)
        self.radii = self._to_full_list(radius, radius_scaling)
        self.nsample = self._to_full_list(nsample, nsample_scaling)
        self.cls_map = cls_map # curvenet
        ...
        if self.cls_map == 'curvenet':
            ...
            self.global_conv2 = nn.Sequential(
                create_convblock1d(fp_channels[-1] * 2, 128, # 2560->1280
                                   norm_args=None,
                                   act_args=act_args))
            self.global_conv1 = nn.Sequential(
                create_convblock1d(fp_channels[-2] * 2, 64, # 1280->64
                                   norm_args=None,
                                   act_args=act_args))
        ...
        for i in range(-1, -n_decoder_stages - 1, -1): # [-1,-2,-3,-4]
            group_args.radius = self.radii[i]
            group_args.nsample = self.nsample[i]
            decoder[i] = self._make_dec( # [160+64+128+16, 320, 640, 1280] [160, 320, 640, 1280]
                skip_channels[i], fp_channels[i], group_args=group_args, block=block, blocks=self.blocks[i])

        self.decoder = nn.Sequential(*decoder)
        self.out_channels = fp_channels[-n_decoder_stages] # 160
    ...
    def _make_dec(self, skip_channels, fp_channels, group_args=None, block=None, blocks=1):
        ...
        layers.append(FeaturePropogation(mlp, act_args=self.act_args, group_args=group_args))
        self.in_channels = fp_channels
        for i in range(1, blocks): # F
            group_args.radius = radii[i]
            group_args.nsample = nsample[i]
            layers.append(block(self.in_channels,
                                aggr_args=self.aggr_args,
    ...
    def forward(self, p, f, cls_label):
        ...
        if self.cls_map == 'curvenet': # T
            ...
            cls_one_hot = torch.zeros((B, self.num_classes), device=p[0].device) # [B,16]
            cls_one_hot = cls_one_hot.scatter_(1, cls_label, 1).unsqueeze(-1) # [B,16,1]
            cls_one_hot = torch.cat((emb1, emb2, cls_one_hot), dim=1) # [B,128+64+16,1]
            cls_one_hot = cls_one_hot.expand(-1, -1, N) # [B,208,N]
        elif self.cls_map == 'pointnet2':
        ...
        for i in range(-1, -len(self.decoder), -1): # [-1,-2,-3]
            f[i - 1] = self.decoder[i][0]([p[i - 1], f[i - 1]], [p[i], f[i]])

        # TODO: study where to add this ? 
        f[-len(self.decoder) - 1] = self.decoder[0][0]([p[1], torch.cat([cls_one_hot, f[1]], 1)], [p[2], f[2]])

        return f[-len(self.decoder) - 1]

@MODELS.register_module()
class DPAtteNetDecoder(nn.Module):
    def __init__(self,
