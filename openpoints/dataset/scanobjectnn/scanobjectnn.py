...
class ScanObjectNNHardest(Dataset):
    ...
    def __init__(self, data_dir, split,
        ...
        craft_h5_name = os.path.join(
            data_dir, f'{slit_name}_objectdataset_augmentedrot_scale75_craft15.h5')
        if not osp.isfile(h5_name):
            raise FileExistsError(
                f'{h5_name} does not exist, please download dataset at first')
        with h5py.File(h5_name, 'r') as f:
            points = np.array(f['data']).astype(np.float32)
            self.labels = np.array(f['label']).astype(int)
        if not osp.isfile(craft_h5_name):
            raise FileExistsError(
                f'{craft_h5_name} does not exist, please get dataset at first')
        with h5py.File(craft_h5_name, 'r') as f:
            craft = np.array(f['craft']).astype(np.float32)
            
        self.data = np.concatenate([points, craft], axis=-1)
        ...
        logging.info(f'Successfully load ScanObjectNN {split} '
                     f'size: {self.data.shape}, num_classes: {self.labels.max()+1}')
    ...
    def __getitem__(self, idx):
        current_points = self.data[idx][:self.num_points]
        label = self.labels[idx]
        if self.partition == 'train':
            np.random.shuffle(current_points)
        data = {'pos': current_points[:, 0:3],
                'craft': current_points[:, 3:18],
                'y': label
                }
